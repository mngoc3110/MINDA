...
\vspace{0.4cm}
% 2 hình vẽ dạng "quyển sách mở" về giao tuyến đồng quy / song song
\begin{center}
	\begin{tabular}{cc}
...
\vspace{0.4cm}
% 3 hình vẽ dạng sách mở a, b, c cuối trang
\begin{center}
	\begin{tabular}{ccc}
...
\end{center}
\vspace{0.4cm}
\noindent\bluecheck\ \textbf{\color{myblue}{+ Hai đường thẳng phân biệt cùng song song với đường thẳng thứ ba thì song song với nhau.}}
\[
	\begin{cases}
		a \neq b      \\
		a \parallel c \\
...
\vspace{0.4cm}
% 3 Hình vẽ vị trí tương đối giữa đường thẳng và mặt phẳng
\begin{center}
	\begin{tabular}{ccc}
...
\begin{center}
	\begin{tikzpicture}[scale=1.5, >=stealth]
		...
		\draw[thick] (TL) -- (V) -- (O) -- (BL) -- cycle;
		\draw[thick] (V) -- (TR) -- (BR) -- (O);
		% Trục giao tuyến d được thể hiện bằng nét đứt ở sau quyển sách
		\draw[dashed, thick] (O) -- (V);
		\draw[dashed, thick] (BL) to[out=-20, in=200] (BR);
		% Đường thẳng song song a ở ngoài
...
\begin{center}
	\begin{tikzpicture}[scale=1.2, >=stealth]
		...
		% Mặt phẳng Q (Đứng) cắt mặt phẳng P
...
% 3 Hình vẽ hình lăng trụ đặt cạnh nhau
\begin{center}
	\begin{tabular}{ccc}
...
\begin{minipage}{0.45\textwidth}
	\begin{center}
		\begin{tikzpicture}[scale=1.2, >=stealth]
			...
			% Đường thẳng a (nét liền phía trên, nét đứt khuất phía dưới)
			\draw[thick] (1.45, 1.4) node[above] {$a$} -- (O);
			\draw[dashed, thick] (O) -- (3.1, -0.1);

			% Đường cao MH vuông góc với mặt phẳng P
			\draw[thick] (M) -- (H);

			% Đường hình chiếu a' (nối H và giao điểm O)
			\draw[thick] (H) -- (O) node[midway, below] {$a'$};

			% Đường thẳng d trong mặt phẳng P
			...
			% Góc vuông tại chân đường cao H
			\draw (1.8, 0.45) -- (1.95, 0.45) -- (1.95, 0.3);

			% Góc vuông giữa d và hình chiếu a' trong phối cảnh 3D
...
\begin{minipage}{0.45\textwidth}
	\begin{center}
		\begin{tikzpicture}[scale=1.2, >=stealth]
			...
			% Đường thẳng a' đi qua O (song song a)
			\draw[thick] (0.5, 0.2) node[left] {$a'$} -- (2.5, 1.2);
			% Đường thẳng b' đi qua O (song song b)
...
\end{itemize}
\vspace{0.2cm}
% Hình 4 được căn giữa rộng rãi phía dưới phần lý thuyết định nghĩa
\begin{center}
	\begin{tikzpicture}[scale=1.5, >=stealth]
		% Mặt phẳng P với nền màu lam mờ hiện đại
		...
		% Đường thẳng d cắt mặt phẳng tại O
		\draw[thick] (1.45, 1.65) node[above] {$d$} -- (O);
		\draw[thick] (O) -- (3.3, -0.2);

		% Đường cao MH vuông góc với P
		\draw[thick] (M) -- (H);

		% Đường hình chiếu d' (nối H và O)
...
\begin{minipage}{0.4\textwidth}
	\begin{center}
		\begin{tikzpicture}[scale=1.4, >=stealth]
			...
			% Góc vuông phối cảnh 3D giữa OM, ON và bờ d tại O
			\draw (0, 0.65) -- (-0.12, 0.755) -- (-0.12, 0.605);
			\draw (0, 0.65) -- (0.18, 0.59) -- (0.18, 0.44);

			% Cung tròn màu đỏ nổi bật biểu thị số đo góc phẳng nhị diện MON
...
\begin{center}
	\begin{tikzpicture}[scale=1.4, >=stealth]
		...
		\draw (0.3,0) arc (0:50:0.3);
		\node at (0.15,0.15) [below left] {$P$};
		% Tọa độ các điểm H và K
		\coordinate (H) at (2.0, 1.7);
		\coordinate (K) at (2.0, 0.5);

		% Đường thẳng a song song mặt phẳng
		\draw[thick] (0.8, 1.7) -- (3.2, 1.7) node[above right] {$a$};

		% Đường hình chiếu a' trong mặt phẳng
		\draw[thick] (1.2, 0.5) -- (3.6, 0.5) node[above right] {$a'$};

		% Đường thẳng b trong mặt phẳng cắt a' tại K
		\draw[thick] (1.0, 0.9) -- (3.0, 0.1) node[below right] {$b$};

		% Đoạn vuông góc chung HK (màu xanh nước biển đậm nổi bật)
		...
		% Góc vuông tại K giữa HK và a'
		\draw (2.0, 0.65) -- (2.15, 0.65) -- (2.15, 0.5);

		% Góc vuông phối cảnh 3D tại K giữa HK và b
...
\begin{center}
	\begin{tikzpicture}[scale=1.4, >=stealth]
		...
		% Đường thẳng đứng a vuông góc mặt phẳng tại H
		...
		% Đoạn vuông góc chung HK (vẽ nét đứt nằm trong mặt phẳng)
		...
		% Góc vuông tại H giữa đường thẳng đứng a và đoạn HK nằm ngang
		\draw (1.6, 0.75) -- (1.75, 0.75) -- (1.75, 0.6);

		% Góc vuông phối cảnh tại K giữa HK và b
