...
def extract_title(content, filename):
    ...
    return f"Bài giảng chuyên sâu {basename.upper()}"

def extract_theory_sections(content):
    sections = re.findall(r'\\(section|subsection|makeheading)\*?\{([^}]+)\}', content)
    clean_sec = [s[1].strip() for s in sections if len(s[1].strip()) > 3]
    return clean_sec[:4] if clean_sec else ["Khái niệm cơ bản", "Tính chất toán học", "Công thức trọng tâm", "Các dạng bài tập"]
...
for filepath in tex_files:
    ...
    theory_list = extract_theory_sections(content)
    
    script_filename = f"gen_{base_name}.py"
    script_path = os.path.join(OUTPUT_DIR, script_filename)
    
    sec1 = theory_list[0] if len(theory_list) > 0 else "Định nghĩa & Bản chất"
    sec2 = theory_list[1] if len(theory_list) > 1 else "Công thức & Chứng minh"
    sec3 = theory_list[2] if len(theory_list) > 2 else "Các trường hợp đặc biệt & Bẫy đề thi"
...
# ==============================================================================
# BÀI GIẢNG LÝ THUYẾT CHUYÊN SÂU: {title.upper()}
...
class Scene1_Intro_And_Concept(VoiceoverScene):
    ...
    def construct(self):
        ...
        with self.voiceover(text="Chào mừng các em đến với bài giảng lý thuyết chuyên sâu về {title}. Ở bài học này, chúng ta không chỉ học vẹt công thức mà sẽ cùng phân tích kỹ bản chất toán học, nguồn gốc các phép biến đổi và các bẫy thường gặp trong đề thi."):
            self.play(Write(header))
            self.play(FadeIn(sub_info))

        # Khái niệm chuyên sâu
        t_concept = Text("1. {sec1}", font_size=20, color=TEAL).next_to(sub_info, DOWN, buff=0.3)
        ...
        with self.voiceover(text="Về mặt lý thuyết, {sec1} đóng vai trò nền tảng. Khi xét một biểu thức hay đồ thị, điều quan trọng nhất là các em phải nắm rõ điều kiện tồn tại, tập xác định và sự phụ thuộc giữa các đại lượng."):
...
class Scene2_Deep_Theory_Proof(VoiceoverScene):
    ...
    def construct(self):
        ...
        title = Text("2. {sec2} & Phân Tích Các Trường Hợp", font_size=22, color=YELLOW).to_edge(UP)
        self.add(title)
        ...
        with self.voiceover(text="Tiếp theo là phần {sec2}. Các em cần chia rõ các trường hợp xảy ra. Đặc biệt lưu ý bẫy đề thi hay gài bẫy ở điều kiện hệ số a khác 0 hoặc tập xác định D. Chúng ta phải kiểm tra kỹ điều kiện trước khi kết luận."):
...
class Scene3_Exercises_Detailed(VoiceoverScene):
    ...
    def construct(self):
        ...
        q1 = Text("Câu 1: Vận dụng lý thuyết giải toán trọng tâm {title}", font_size=18).next_to(title, DOWN, buff=0.3)
        opts1 = VGroup(
...
class FullLesson_Deep_{base_name.upper()}(Scene1_Intro_And_Concept, Scene2_Deep_Theory_Proof, Scene3_Exercises_Detailed):
    def construct(self):
...
print("✅ Đã nâng cấp toàn bộ 32 Manim scripts sang phiên bản GIẢNG GIẢI LÝ THUYẾT CHUYÊN SÂU!")
