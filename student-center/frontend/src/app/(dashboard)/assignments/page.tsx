  ...
  const fetchSubmissions = async () => {
    try {
      ...
      const [submissionsRes, assignmentsRes, coursesRes] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API_URL || `${process.env.NEXT_PUBLIC_API_URL || '${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}'}`}/api/assignments/teacher/dashboard/submissions`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(`${process.env.NEXT_PUBLIC_API_URL || `${process.env.NEXT_PUBLIC_API_URL || '${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}'}`}/api/assignments/teacher/dashboard/assignments`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(`${process.env.NEXT_PUBLIC_API_URL || `${process.env.NEXT_PUBLIC_API_URL || '${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}'}`}/api/courses/`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
  ...
  return (
    <div className="min-h-screen bg-bg-main text-white p-6 md:p-8 font-outfit selection:bg-pink-500/30">
      ...
      <header className="flex items-center justify-between mb-8 pb-6 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm">Giao bài tập về nhà, luyện thi và chấm điểm cho học viên</p>
          </div>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white/2 border border-white/10 p-6 rounded-3xl flex items-center justify-between">
          <div>
            <p className="text-gray-400 text-sm font-medium mb-1">Đã Tạo</p>
            <p className="text-3xl font-bold text-white">{assignments.length} Đề</p>
          </div>
          <div className="w-12 h-12 rounded-full bg-blue-500/10 flex items-center justify-center border border-blue-500/20">
            <FileText className="text-blue-400 w-6 h-6" />
          </div>
        </div>
        <div className="bg-white/2 border border-white/10 p-6 rounded-3xl flex items-center justify-between relative overflow-hidden">
          <div className="absolute inset-0 bg-orange-500/5 blur-xl pointer-events-none" />
          <div className="relative z-10">
            <p className="text-gray-400 text-sm font-medium mb-1">Cần chấm điểm</p>
        ...
        <div className="bg-white/2 border border-white/10 p-6 rounded-3xl flex items-center justify-between">
          <div>
            <p className="text-gray-400 text-sm font-medium mb-1">Đã chấm xong</p>
            <p className="text-3xl font-bold text-white">{submissions.filter((s) => s.status !== "pending").length}</p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        {/* Assignment list */}
        <div className="lg:col-span-2 bg-[#0a0a0a] rounded-3xl border border-white/10 overflow-hidden flex flex-col max-h-[600px]">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="text-sm">
                {loading ? (
                  ...
                  <tr><td colSpan={4} className="text-center p-8 text-gray-500">Sếp chưa giao bài tập nào!</td></tr>
                  ...
                  <tr key={item.id} className="border-b border-white/5 hover:bg-white/2 transition-colors">
                    ...
                    <td className="p-4 text-gray-400">{item.assignment_type === "quiz" ? "Trắc nghiệm" : "Tự luận"}</td>
                    <td className="p-4 text-gray-400">{new Date(item.created_at).toLocaleDateString("vi-VN")}</td>
                    <td className="p-4">
                      {item.course_id ? (
                        <span className="text-xs px-2 py-1 bg-white/5 rounded text-gray-400 border border-white/10">Khoá học</span>
                      ) : (
        ...
        {/* Submissions grouped */}
        <div className="lg:col-span-3 bg-[#0a0a0a] rounded-3xl border border-white/10 overflow-hidden flex flex-col max-h-[600px]">
          <div className="flex items-center justify-between p-6 border-b border-white/10 shrink-0">
            <h2 className="text-lg font-bold flex items-center gap-2">
              <Clock className="w-5 h-5 text-orange-400" /> Kết quả học sinh
              <span className="text-xs font-normal text-gray-500 ml-1">— điểm cao nhất</span>
            </h2>
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-4 py-2 bg-white/5 border border-white/10 rounded-lg text-sm focus:outline-none focus:border-orange-500/50"
              />
            </div>
          </div>
          ...
          <div className="overflow-y-auto flex-1">
            {loading ? (
              ...
              <div className="text-center p-8 text-gray-500">Chưa có học sinh nào nộp bài.</div>
            ) : grouped.map((g) => {
              ...
              return (
                <div key={key} className="border-b border-white/5 last:border-0">
                  {/* Summary row */}
                  <div
                    className={`flex items-center gap-3 p-4 pl-6 pr-5 transition-colors ${canExpand ? "cursor-pointer hover:bg-white/2" : ""}`}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-sm text-white truncate">{g.student_name}</p>
                      <p className="text-xs text-gray-500 truncate">{g.assignment_title}</p>
                    </div>
                  ...
                  {isOpen && (
                    <div className="bg-white/1.5 border-t border-white/5">
                      <p className="text-xs text-gray-500 font-bold uppercase tracking-wider px-6 pt-3 pb-1">
                        Lịch sử {g.attempt_count} lần làm
                      </p>
                      {g.attempts.map((att, idx) => (
                        <div
                          key={att.id}
                          className="flex items-center gap-3 px-6 py-2.5 border-b border-white/5 last:border-0 last:pb-3"
                        >
