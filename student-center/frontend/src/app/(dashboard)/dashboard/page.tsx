...
export default function Dashboard() {
  ...
  const renderDriveSection = (accentColor: string) => (
    <section className="flex-1 p-6 rounded-3xl bg-white border border-[#E2D9CE] flex flex-col min-h-[260px] shadow-sm">
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-base font-bold flex items-center gap-2 text-[#1A1410]">
          <HardDrive className={`w-5 h-5 ${accentColor}`} /> MINDA_Storage Của Tôi
        </h2>
        <span className="text-xs text-[#5C4F42] font-medium">{files.length} Tệp</span>
      </div>
      
      {files.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center opacity-40 gap-2">
          <HardDrive className="w-10 h-10 text-[#5C4F42]" />
          <p className="text-sm text-[#5C4F42]">Trống. Hãy tải lên tài liệu đầu tiên!</p>
        </div>
      ) : (
        <div className="space-y-2 flex-1 overflow-y-auto max-h-[220px] pr-1">
          {files.map((file) => (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center justify-between p-3 rounded-xl bg-[#F7F3EE] hover:bg-[#EEE9E1] border border-[#E2D9CE] transition-all cursor-pointer group"
            >
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-lg bg-white border border-[#E2D9CE] flex items-center justify-center">
                  <FileText className="w-4 h-4 text-[#5C4F42]" />
                </div>
                <div>
                  <span className="text-sm font-semibold truncate block w-32 md:w-48 text-[#1A1410]">{file.filename}</span>
                  <span className="text-[10px] text-[#5C4F42] uppercase tracking-widest">{file.file_type.split('/')[1] || 'FILE'}</span>
                </div>
              </div>
              <div className="flex flex-col items-end gap-1">
                <span className="text-xs text-[#5C4F42]">{file.file_size}</span>
      ...
      <button
        ...
        className="w-full mt-4 py-3 rounded-xl bg-[#F7F3EE] hover:bg-[#EEE9E1] border border-[#E2D9CE] text-sm font-semibold text-[#1A1410] transition-colors flex items-center justify-center gap-2 disabled:opacity-40"
      >
        {uploading ? (
  ...
  if (role === "teacher") {
    return (
      <div className="min-h-screen bg-bg-main p-6 md:p-8 font-outfit">
        <header className="flex items-center justify-between mb-8 pb-6 border-b border-[#E2D9CE]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-xl tracking-tight leading-none mb-0.5 text-[#1A1410]">Teacher Dashboard</h1>
              <p className="text-[#5C4F42] text-sm">Chào mừng thầy/cô <strong>{userName}</strong>!</p>
            </div>
          </div>
          <Link href="/login" onClick={() => localStorage.clear()} className="px-4 py-2 border border-[#E2D9CE] hover:bg-[#EEE9E1] rounded-full text-sm font-semibold text-[#1A1410] transition-colors">
            Đăng xuất
          </Link>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
          {[
            ...
            <div key={i} className={`p-5 rounded-2xl bg-white border ${stat.border} flex items-center gap-4 shadow-sm`}>
              ...
              <div>
                <p className="text-xs text-[#5C4F42] font-medium mb-0.5">{stat.label}</p>
                <p className="text-2xl font-black text-[#1A1410]">{stat.value as string}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <section className="p-6 rounded-3xl bg-white border border-[#E2D9CE] shadow-sm">
    ...
    <div className="min-h-screen bg-bg-main p-6 md:p-8 font-outfit">
      <header className="flex items-center justify-between mb-8 pb-6 border-b border-[#E2D9CE]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-xl tracking-tight leading-none mb-0.5 text-[#1A1410]">MINDA Workspace</h1>
            <p className="text-[#5C4F42] text-sm">Chào mừng <strong>{userName}</strong> quay trở lại!</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 flex flex-col gap-5">
          ...
          <div className="relative p-[2px] rounded-3xl bg-gradient-to-br from-indigo-400/50 via-purple-300/20 to-transparent overflow-hidden shadow-lg">
            <div className="relative bg-white rounded-[22px] p-6 flex flex-col border border-[#E2D9CE]">
              <div className="flex items-center justify-between mb-5">
                ...
                <div className="text-right">
                  <p className="text-[10px] text-[#5C4F42] uppercase tracking-widest font-bold">MINDA.EDU ID</p>
                  <p className="text-sm font-mono text-indigo-600 tracking-wider font-bold">{stats.student_id as string || "#MND-0000"}</p>
                </div>
              </div>
              
              <div className="mb-5">
                <h3 className="font-black text-xl tracking-tight text-[#1A1410] mb-1 truncate">{userName}</h3>
                <p className="text-sm text-[#5C4F42] flex items-center gap-2">
              ...
              <div className="mt-auto space-y-2.5">
                <div className="p-3 rounded-xl bg-[#F7F3EE] border border-[#E2D9CE] flex items-center justify-between">
                  <span className="text-xs text-[#5C4F42] uppercase font-bold tracking-widest">Trạng thái</span>
          ...
          <section className="p-6 rounded-3xl bg-white border border-[#E2D9CE] shadow-sm">
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h3 className="font-bold text-lg text-[#1A1410]">Sơ cấp</h3>
                <p className="text-sm text-[#5C4F42]">Hoàn thành bài tập để thăng hạng</p>
              </div>
            </div>
            <div className="w-full bg-[#EEE9E1] rounded-full h-2 mb-2">
              <div className="bg-gradient-to-r from-slate-400 to-slate-600 h-2 rounded-full w-0"></div>
            </div>
            <p className="text-xs text-center text-[#5C4F42]">Hoàn thành 1 bài tập hoặc 1 khoá học để nhận ⭐ và điểm EXP!</p>
          </section>
