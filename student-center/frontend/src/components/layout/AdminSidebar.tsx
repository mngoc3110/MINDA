...
export default function AdminSidebar() {
  ...
  return (
    <aside className="w-[280px] h-screen border-r border-white-[0.08] bg-bg-main/80 backdrop-blur-3xl flex-col hidden lg:flex shrink-0 relative z-50">
      {/* Hiệu ứng ánh sáng */}
      <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-b from-red-500/5 via-transparent to-transparent -z-10 pointer-events-none" />
      ...
      <Link href="/" className="p-8 pb-6 flex items-center gap-4 hover:opacity-80 transition-opacity">
        ...
        <div>
           ...
           <span className="text-xs text-red-400 font-semibold tracking-widest uppercase mt-1 block">Admin Console</span>
        </div>
      </Link>
      ...
      <div className="p-5 border-t border-white-[0.05]">
        <div className="p-4 rounded-2xl bg-gradient-to-b from-white/[0.04] to-transparent border border-white/[0.06] mb-4 flex gap-4 items-center">
             <div className="w-12 h-12 rounded-full border-2 border-red-500/50 p-0.5 shrink-0 shadow-[0_0_15px_rgba(220,38,38,0.3)]">
                <div className="w-full h-full rounded-full bg-gradient-to-br from-red-500 to-orange-600 flex items-center justify-center">
                    <span className="text-white font-bold text-lg">🛡️</span>
                </div>
             </div>
             <div className="flex-1 min-w-0">
               <p className="text-sm font-bold text-white truncate">Administrator</p>
               <p className="text-[11px] text-red-400 mt-0.5 font-bold tracking-wide uppercase">Root Access</p>
             </div>
        </div>
