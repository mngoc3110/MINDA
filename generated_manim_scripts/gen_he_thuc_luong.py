...
# ==============================================================================
# BÀI GIẢNG LÝ THUYẾT CHUYÊN SÂU: HỆ THỨC LƯỢNG TRONG TAM GIÁC
...
class Scene1_Intro_And_Concept(VoiceoverScene):
    ...
    def construct(self):
        ...
        header = Text("HỆ THỨC LƯỢNG TRONG TAM GIÁC", font_size=24, color=YELLOW).to_edge(UP)
        sub_info = Text("CHUYÊN ĐỀ TỔNG ÔN CHUYÊN SÂU | GV: NGUYỄN LÊ MINH NGỌC", font_size=16, color=BLUE_B).next_to(header, DOWN, buff=0.2)

        with self.voiceover(text="Chào mừng các em đến với bài giảng lý thuyết chuyên sâu về Hệ thức lượng trong tam giác. Ở bài học này, chúng ta không chỉ học vẹt công thức mà sẽ cùng phân tích kỹ bản chất toán học, nguồn gốc các phép biến đổi và các bẫy thường gặp trong đề thi."):
            self.play(Write(header))
            self.play(FadeIn(sub_info))

        # Khái niệm chuyên sâu
        t_concept = Text("1. Giá trị lượng giác của một góc", font_size=20, color=TEAL).next_to(sub_info, DOWN, buff=0.3)
        ...
        with self.voiceover(text="Về mặt lý thuyết, Giá trị lượng giác của một góc đóng vai trò nền tảng. Khi xét một biểu thức hay đồ thị, điều quan trọng nhất là các em phải nắm rõ điều kiện tồn tại, tập xác định và sự phụ thuộc giữa các đại lượng."):
...
class Scene2_Deep_Theory_Proof(VoiceoverScene):
    ...
    def construct(self):
        ...
        title = Text("2. \blackbox{2 & Phân Tích Các Trường Hợp", font_size=22, color=YELLOW).to_edge(UP)
        self.add(title)
        ...
        with self.voiceover(text="Tiếp theo là phần \blackbox{2. Các em cần chia rõ các trường hợp xảy ra. Đặc biệt lưu ý bẫy đề thi hay gài bẫy ở điều kiện hệ số a khác 0 hoặc tập xác định D. Chúng ta phải kiểm tra kỹ điều kiện trước khi kết luận."):
...
class Scene3_Exercises_Detailed(VoiceoverScene):
    ...
    def construct(self):
        ...
        q1 = Text("Câu 1: Vận dụng lý thuyết giải toán trọng tâm Hệ thức lượng trong tam giác", font_size=18).next_to(title, DOWN, buff=0.3)
        opts1 = VGroup(
...
class FullLesson_Deep_HE_THUC_LUONG(Scene1_Intro_And_Concept, Scene2_Deep_Theory_Proof, Scene3_Exercises_Detailed):
    def construct(self):
