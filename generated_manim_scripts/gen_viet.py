...
class Scene1_Intro_And_Concept(VoiceoverScene):
    ...
    def construct(self):
        ...
        t_concept = Text("1. Định nghĩa", font_size=20, color=TEAL).next_to(sub_info, DOWN, buff=0.3)
        ...
        with self.voiceover(text="Về mặt lý thuyết, Định nghĩa đóng vai trò nền tảng. Khi xét một biểu thức hay đồ thị, điều quan trọng nhất là các em phải nắm rõ điều kiện tồn tại, tập xác định và sự phụ thuộc giữa các đại lượng."):
...
class Scene2_Deep_Theory_Proof(VoiceoverScene):
    ...
    def construct(self):
        ...
        title = Text("2. Một số dạng đặc biệt & Phân Tích Các Trường Hợp", font_size=22, color=YELLOW).to_edge(UP)
        self.add(title)
        ...
        with self.voiceover(text="Tiếp theo là phần Một số dạng đặc biệt. Các em cần chia rõ các trường hợp xảy ra. Đặc biệt lưu ý bẫy đề thi hay gài bẫy ở điều kiện hệ số a khác 0 hoặc tập xác định D. Chúng ta phải kiểm tra kỹ điều kiện trước khi kết luận."):
...
class FullLesson_Deep_VIET(Scene1_Intro_And_Concept, Scene2_Deep_Theory_Proof, Scene3_Exercises_Detailed):
    def construct(self):
