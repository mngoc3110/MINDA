...
# ==============================================================================
# BÀI GIẢNG LÝ THUYẾT CHUYÊN SÂU: GIỚI HẠN VÀ ĐẠO HÀM
...
class Scene1_Intro_And_Concept(VoiceoverScene):
    ...
    def construct(self):
        ...
        header = Text("GIỚI HẠN VÀ ĐẠO HÀM", font_size=24, color=YELLOW).to_edge(UP)
        sub_info = Text("CHUYÊN ĐỀ TỔNG ÔN CHUYÊN SÂU | GV: NGUYỄN LÊ MINH NGỌC", font_size=16, color=BLUE_B).next_to(header, DOWN, buff=0.2)

        with self.voiceover(text="Chào mừng các em đến với bài giảng lý thuyết chuyên sâu về Giới hạn và đạo hàm. Ở bài học này, chúng ta không chỉ học vẹt công thức mà sẽ cùng phân tích kỹ bản chất toán học, nguồn gốc các phép biến đổi và các bẫy thường gặp trong đề thi."):
...
class Scene3_Exercises_Detailed(VoiceoverScene):
    ...
    def construct(self):
        ...
        q1 = Text("Câu 1: Vận dụng lý thuyết giải toán trọng tâm Giới hạn và đạo hàm", font_size=18).next_to(title, DOWN, buff=0.3)
        opts1 = VGroup(
...
class FullLesson_Deep_LIM_DAOHAM(Scene1_Intro_And_Concept, Scene2_Deep_Theory_Proof, Scene3_Exercises_Detailed):
    def construct(self):
